...
krt_do_scan(struct krt_proto *p UNUSED)	/* CONFIG_ALL_TABLES_AT_ONCE => p is NULL */
{
  ...
  nl_request_dump(AF_INET, RTM_GETROUTE, NL_PID_KERNEL);
  while (h = nl_get_scan())
  ...
  nl_request_dump(AF_INET6, RTM_GETROUTE, NL_PID_KERNEL);
  while (h = nl_get_scan())
    if (h->nlmsg_type == RTM_NEWROUTE || h->nlmsg_type == RTM_DELROUTE)
      nl_parse_route(&s, h);
    else
...
#ifdef HAVE_MPLS_KERNEL
  nl_parse_begin(&s, 1, 1);
  nl_request_dump(AF_MPLS, RTM_GETROUTE, NL_PID_KERNEL);
  while (h = nl_get_scan())
    if (h->nlmsg_type == RTM_NEWROUTE || h->nlmsg_type == RTM_DELROUTE)
      nl_parse_route(&s, h);
    else
